...
class Kohonen:
    ...
    def log_progess(self, current_epoch):
        ...
        if current_epoch == self.epochs:
            print('[' + int(bar_count) * '=' + '>' + '.' * int(50 - bar_count) + f'] {current_epoch} / {self.epochs}',
                  end="\n")
        else:
            print('[' + int(bar_count) * '=' + '>' + '.' * int(50 - bar_count) + f'] {current_epoch} / {self.epochs}',
                  end="\r")
    ...
    def train(self):
        for epoch_count in range(self.epochs):
            ...
            for idx, i in enumerate(self.traindata):
                ...
                # Find bmu_node
                distances = list(chain.from_iterable([[sqrt(
                    reduce(lambda a, b: a + b, [pow(k - m.prototype[idx], 2) for idx, k in enumerate(i)])) for m in j]
                ...
                # Find neighbors and update neighbors + bmu
                neighborhood_r = (self.n / 2) * (1 - (epoch_count / self.epochs))
                for node in list(chain.from_iterable(self.clusters)):
                    node_to_bmu = sqrt(reduce(lambda a, b: a + b, [pow(k - node.location[idx], 2) for idx, k in
                                                                   enumerate(bmu_node.location)]))
                    if node_to_bmu <= neighborhood_r:
                        node.prototype = [(1 - self.initial_learning_rate) * j + self.initial_learning_rate * i[idx2]
                                          for idx2, j in enumerate(node.prototype)]
